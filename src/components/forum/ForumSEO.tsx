...
export const ForumSEO = ({ activeCategory, bannerConfig }: ForumSEOProps) => {
  ...
  if (activeCategory) {
    ...
    if (activeCategory.seo_title && activeCategory.seo_title.trim().length > 0) {
      ...
      title = `${activeCategory.name} Forum - Erfahrungen & Diskussionen | Rank-Scout`;
    }
    ...
    // Notfall-Fallback
      ...
      description = `Willkommen im Bereich ${activeCategory.name}. Diskutiere jetzt mit der Rank-Scout Community über ${activeCategory.name}.`;
    }
  } 
  // 2. Startseite (Forum Home)
  else {
    if (bannerConfig?.headline) title = `${bannerConfig.headline} | Rank-Scout`;
