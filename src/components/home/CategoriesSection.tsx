...
export const CategoriesSection = () => {
  ...
  const limit = content.categories.count || 6; // Dynamischer Limit aus Settings
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="mb-12 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          ...
          <Button variant="ghost" className="hidden sm:flex gap-2 text-muted-foreground hover:text-primary" asChild>
            <Link to="/kategorien">
              {content.categories.button_more || "Alle anzeigen"} <ArrowRight className="w-4 h-4" />
            </Link>
          </Button>
        </div>
        ...
        <div className="mt-10 text-center sm:hidden">
          <Button className="w-full bg-blue-950 text-white" asChild>
            <Link to="/kategorien">{content.categories.button_more || "Alle anzeigen"}</Link>
          </Button>
        </div>
      </div>
    </section>
