...
const EMPTY_CATEGORY = {
  ...
  ad_link_url: "",
  ad_html_code: "", 
  ad_headline: "", // Wieder da
  ad_subheadline: "", // Wieder da
  ad_cta_text: "" // Wieder da
};
...
export default function AdminForum() {
  ...
  const handleEditThread = (thread: ForumThread) => {
    ...
    setFormData({
      title: thread.title, slug: thread.slug, content: thread.content, raw_html_content: thread.raw_html_content || "",
      author_name: thread.author_name, seo_title: thread.seo_title || "", seo_description: thread.seo_description || "",
      featured_image_url: thread.featured_image_url || "", is_pinned: thread.is_pinned || false, is_locked: thread.is_locked || false,
      is_answered: thread.is_answered || false, is_active: thread.is_active !== false, admin_notes: thread.admin_notes || "",
      status: thread.status || "published", category_id: thread.category_id || null,
    });
    setEditorOpen(true);
  };
  ...
  const handleSaveThread = async () => {
    ...
    try {
      if (editingThread?.id) { await updateThread.mutateAsync({ id: editingThread.id, ...formData, }); toast.success("Beitrag aktualisiert"); } else { await createThread.mutateAsync(formData); toast.success("Beitrag erstellt"); }
      setEditorOpen(false);
  ...
  const handleEditCategory = (category: ForumCategory) => {
    ...
    setCategoryFormData({
      name: category.name, slug: category.slug, description: category.description || "", sort_order: category.sort_order, is_active: category.is_active,
      // Ad Data
  ...
  return (
    <div className="space-y-6">
      ...
      <Dialog open={editorOpen} onOpenChange={setEditorOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap gap-6"><div className="flex items-center gap-2"><Switch checked={formData.is_pinned} onCheckedChange={(v) => setFormData((p) => ({ ...p, is_pinned: v }))} /><Label>Angepinnt</Label></div><div className="flex items-center gap-2"><Switch checked={formData.is_locked} onCheckedChange={(v) => setFormData((p) => ({ ...p, is_locked: v }))} /><Label>Geschlossen</Label></div><div className="flex items-center gap-2"><Switch checked={formData.is_answered} onCheckedChange={(v) => setFormData((p) => ({ ...p, is_answered: v }))} /><Label>Beantwortet</Label></div><div className="flex items-center gap-2"><Switch checked={formData.is_active} onCheckedChange={(v) => setFormData((p) => ({ ...p, is_active: v }))} /><Label>Veröffentlicht</Label></div></div>
      ...
      {/* CATEGORY EDITOR DIALOG (MIT WERBUNG) */}
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto">
          ...
          <Tabs defaultValue="basic">
            ...
            <TabsContent value="basic" className="space-y-4 pt-4">
                ...
                <div><Label>Beschreibung</Label><Textarea value={categoryFormData.description} onChange={(e) => setCategoryFormData((p) => ({ ...p, description: e.target.value }))} rows={2} placeholder="Kurze Beschreibung..." /></div>
